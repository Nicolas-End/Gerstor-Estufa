...
export function DeliveryQuantidy(): Promise<ApiResponse> {
    return new Promise<ApiResponse>((resolve) =>{
        ...
        axios.post<ApiResponse>('https://gerenciador-empresarial-1cfr.vercel.app/count-deliverys', {
...
export function AddNewDelivery (FormsData:any): Promise<ApiResponse> {
    return new Promise<ApiResponse>((resolve) =>{
        const token = localStorage.getItem('token_from_user')
        axios.post<ApiResponse>('https://gerenciador-empresarial-1cfr.vercel.app/add-new-delivery', {
            FormsData
          },{
            headers:{
                'Authorization':  token || ''
