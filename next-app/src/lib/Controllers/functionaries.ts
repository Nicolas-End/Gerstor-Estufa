...
export function GetFunctionaries(): Promise<ApiResponse> {
    return new Promise<ApiResponse>((resolve) => {
      ...
      axios.post<ApiResponse>(
        'https://gerenciador-empresarial-1cfr.vercel.app/get-functionaries',
  ...
  export function GetFunctionariesQuantity(): Promise<ApiResponse> {
    return new Promise<ApiResponse>((resolve) => {
      ...
      const token = localStorage.getItem('token_from_user')
      axios.post<ApiResponse>(
        'https://gerenciador-empresarial-1cfr.vercel.app/get-functionaries-quantity',
        {}, // corpo da requisição POST (vazio nesse caso)
        {
          headers: {
            'Authorization':  token || ''
    ...
    return new Promise<ApiResponse>((resolve) => {
      ...
      const token = localStorage.getItem('token_from_user')
      axios.post<ApiResponse>(
        'https://gerenciador-empresarial-1cfr.vercel.app/add-new-functionary',
        {"name":name,
          "email":email,
          "password":password,
          "role":role
